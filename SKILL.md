...
1. **Lire l'entrée.** Texte collé : tel quel. Fichier `.txt`, `.md`, `.pdf` : le lire. Fichier `.docx` : `python -c "import docx,sys; print('\n'.join(p.text for p in docx.Document(sys.argv[1]).paragraphs))" "<chemin>"`. Lien Google Drive ou Notion : connecteur correspondant ; autre lien : récupérer la page. Entrée vide : prendre le texte fourni dans la conversation, sinon le demander. Entrée illisible ou inaccessible : le dire, demander de coller le texte, s'arrêter.
2. **Destination.** Base `${user_config.airtable_base_id}`, table `${user_config.airtable_table_id}`. Une valeur vide ou commençant par `${` n'est pas configurée : utiliser la destination par défaut. Destination configurée différente de la destination par défaut : appliquer Autre destination, étapes 2 à 4 (correspondance à valider) ; les identifiants de champs ci-dessous ne valent que pour la destination par défaut. Une seule des deux valeurs configurée : demander. Si l'utilisateur désigne une autre base (« dans la base X ») : section Autre destination.
...
7. **Aperçu** (section Aperçu). Attendre la validation. Seul « oui » déclenche la création. Après « modifier n°X » ou « retirer n°Y » : nouvel aperçu. Une tâche signalée en doublon n'est créée que si « Créer quand même ? » a reçu « oui » pour elle ; sinon, redemander.
...
- **Projet** : un seul, cherché par nom parmi tous les projets, en priorité ceux qui ne sont pas clos. Statuts clos : `Annulé`, `Terminé`, `Done`, `A supprimer`. Seule correspondance close, ou plusieurs candidats : question.
...
Avant les questions, pour chaque projet retenu ou candidat, lire ses tâches existantes, tous statuts confondus. Tâche identique ou proche : signaler l'existante (titre, statut, projet) et demander « Créer quand même ? (oui/non) » dès la première réponse : dans la liste de questions s'il y en a, sinon dans l'aperçu. La reprendre dans la colonne Doublon de l'aperçu, avec le projet de la tâche existante. Projet ou contenu fixé par une réponse : comparer à nouveau avant l'aperçu. Aucun choix par défaut : rien n'est créé pour cette tâche sans réponse. Les tâches existantes servent aussi de contexte, sans rien ajouter qui ne soit dans le texte.
...
|---|---|---|---|---|---|---|
| 1 | Rédiger les mentions légales | Aurel | Site NovekAI Workforce | To Do | 16/09/2026 | — |
...
2. [ ] Remplacer les anciens prix · [ ] Vérifier l'affichage mobile
   ...
   Source : « Hugues met à jour la page tarifs : remplacer les anciens prix, vérifier l'affichage mobile — maquette https://exemple.fr/tarifs »

Tâche 2 : une tâche proche existe. Créer quand même ? (oui/non)
Valider ? (oui / modifier n°X / retirer n°Y)
```

L'aperçu montre tout ce qui sera écrit : statut dans le tableau, ligne `Infos :` sous les étapes des tâches qui en ont une. Colonne Doublon : titre, statut et projet de la tâche existante.
...
1. Chercher la base par son nom. Base au nom exact : la retenir, sans demander de confirmation. Plusieurs résultats : demander laquelle. Un seul résultat au nom approchant : demander de confirmer. Aucun résultat : le dire, demander le nom exact ou l'identifiant de la base, s'arrêter là.
